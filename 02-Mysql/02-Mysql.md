...
虽然我无法直接画图，但我可以用**文字图示**和**层级结构**为你清晰地展示 MySQL 数据源（DataSource）与数据库连接池（Connection Pool）之间的关系。
...
| 辅助索引存储内容 | 叶子节点存**主键值**        | 叶子节点存**行数据的物理地址**           |
...
对于普通的 `SELECT` 语句（即**快照读**），MySQL 通过 MVCC 机制来避免幻读[[source_group_web_2]]。
...
- 核心原理：Next-Key Lock 是 **记录锁（Record Lock）和间隙锁（Gap Lock）**的组合[[source_group_web_6]]。
  - **记录锁**：锁定索引记录本身。
  - **间隙锁**：锁定索引记录之间的“间隙”，防止其他事务在该间隙中插入新记录[[source_group_web_7]]。
- **工作方式**：当执行一个范围条件的当前读时，InnoDB 不仅会锁定符合条件的行，还会锁定这些行的索引范围，阻止其他事务向该范围插入新数据[[source_group_web_8]]。
...
在这个过程中，事务A先查不到数据，后又查到了数据，且更新了之前“看不见”的数据，这本质上就是一种幻读[[source_group_web_11]]。这种情况的发生，根源在于**快照读**和**当前读**所遵循的规则不同[[source_group_web_12]]。
...
当你执行一条 SQL 语句（例如 `SELECT * FROM users WHERE id = 29`）时，MySQL 利用 B+ 树索引的过程如下：[[source_group_web_5]]

1. **从根节点开始**：MySQL 从 B+ 树的根节点（存储在数据字典中）开始查找[[source_group_web_6]]。
2. 逐层向下定位：
...
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **聚簇索引** (主键索引) | 叶子节点存储**完整的数据行**[[source_group_web_8]]。表数据本身就是按主键组织的 B+ 树。 | 1. 从根节点遍历到叶子节点。 2. **直接**在叶子节点中读取完整的行数据（无需回表）[[source_group_web_9]]。 |
| **二级索引** (辅助索引) | 叶子节点存储**索引列的值 + 主键值**[[source_group_web_10]]。 | 1. 在二级索引树中找到对应的主键值（如 id=29）。 2. **回表查询**：拿着这个主键值，再去聚簇索引中查找完整数据[[source_group_web_11]]。 |
...
- 范围查询
  ...
  ：B+ 树的优势在此体现得淋漓尽致[[source_group_web_12]]。

  1. 先定位到范围的起始点（例如 `id >= 20`）。
  2. 利用叶子节点之间的链表指针，**顺序向右扫描**，直到遇到不符合条件的记录（例如 `id > 30`）为止。这种顺序 I/O 比随机 I/O 快得多[[source_group_web_13]]。
