...
![JVM运行时数据区1](01-JVM.assets/JVM运行时数据区1.png)
...
- **方法区**：是**JVM 规范里的逻辑概念**，是抽象定义，规定要存什么数据；
  - **永久代（PermGen）**：是**JDK1.7 及之前 HotSpot 对方法区的具体实现**；
  - **元空间（Metaspace）**：是**JDK1.8 及之后 HotSpot 替代永久代、实现方法区**。
  
  一句话总结：
  
  **方法区是规范抽象 → JDK8 之前用永久代实现 → JDK8 之后彻底废弃永久代，改用元空间实现。**
...
**类加载**：把 `.class` 二进制文件数据读到内存，转换成**运行时数据区的 Class 对象**，并做校验、准备、解析、初始化的全过程。
...
1. 通过**全类名**获取 `.class` 二进制字节流
2. 将字节流转为**方法区运行时数据结构**
3. 在内存堆中生成 **java.lang.Class 对象** 作为入口
...
- 给**类静态变量**分配内存
- 设置**默认初始值**（int=0，boolean=false，引用=null）
- **不赋代码里的初始值**，常量直接赋最终值
...
#### 三、类初始化 6 种触发时机（主动引用，必会初始化）

1. new 创建对象
2. 调用类的静态方法
3. 使用类/接口的静态常量（非编译期常量）
4. 反射 `Class.forName()`
5. 启动入口 main 类
6. 子类初始化，先触发父类初始化
...
1. 类加载收到请求，**先不自己加载**
2. 向上委托给**父加载器**
3. 父加载器再往上委托，直到**启动类加载器**
4. 顶层加载不了，再**向下由子类加载器尝试加载**

**一句话**：**向上委托，向下查找**

##### 3. 为什么要双亲委派？

1. **沙箱安全**：防止自定义 `java.lang.String` 篡改核心类
2. **类全局唯一**：保证全类名只加载一份，避免重复加载
3. 统一类加载规则，稳定可靠
...
1. **SPI 服务提供者接口**（JDBC、Dubbo）
   线程上下文类加载器反向加载
2. **热部署、热加载**
3. **自定义类加载器** 重写 `loadClass()` 不走委派
...
1. 类加载五步：**加验准解初**
2. 初始化只执行一次，静态变量、静态代码块在这里赋值
3. 三加载器：**启动、扩展、应用**
4. 双亲委派：**向上委托，向下加载**，防篡改、保唯一
5. 6种主动引用触发初始化，被动不触发

需要我给你出 **3道经典JVM类加载面试真题+标准回答** 直接背吗？
...
JVM 垃圾回收（GC，Garbage Collection）是**JVM自动回收不再使用的内存空间**的机制，目的是解决手动管理内存（如C/C++的malloc/free）容易出现的**内存泄漏、内存溢出**问题，让开发者专注业务逻辑。

简单说：**JVM会自动找到程序里“没用的对象”，把它们清理掉，腾出内存给新对象用**。
...
GC 只干三件事：
1. **找到垃圾**：哪些内存/对象是程序不再使用的？
2. **清理垃圾**：把无用对象占用的内存释放掉；
3. **内存整理**：避免内存碎片化，保证连续可用内存。
...
- 给每个对象加一个计数器，被引用一次+1，引用失效一次-1；
- 计数器=0 → 判定为垃圾。
- **缺陷**：无法解决**循环引用**（A引用B，B引用A，互相引用但都没用了，计数器永远不为0），JVM**不使用**。

##### 2. 可达性分析算法（JVM 标准算法）

- 以**GC Roots**（根对象）为起点，向下搜索引用链；
- 能被GC Roots 关联到 → 存活对象；
- 无法到达 → 判定为垃圾。
...
- **流程**：先标记所有垃圾 → 统一清除。
- **优点**：简单，无需移动对象。
- **缺点**：产生大量**内存碎片**（内存不连续），大对象无法分配。

##### 2. 标记-复制算法（Mark-Copy）

- **流程**：把内存分成两块（如From/To），只使用一块；存活对象复制到另一块 → 清空原内存。
- **优点**：无内存碎片，速度快。
- **缺点**：内存利用率只有50%。
- **适用**：存活对象少的场景（新生代）。
...
#### 四、核心设计：分代回收（JVM 实际采用）

JVM 根据**对象存活时间**，把堆内存分为**新生代**和**老年代**，不同区域用不同GC算法，这就是**分代回收**。

##### 1. 堆内存划分

```
堆内存 = 新生代 + 老年代 + 元空间（永久代替代）
新生代 = Eden区 + 两个Survivor区（S0/S1，大小相等）
```
...
- 所有新对象优先在**Eden区**创建；
- Eden区满 → 触发**Minor GC（轻量GC）**；
- 使用**标记-复制算法**：
  1. 清理Eden+S0，存活对象复制到S1；
  2. 交换S0/S1，保证总有一个Survivor为空；
  3. 对象在Survivor每熬过一次GC，年龄+1；
  4. 年龄达到阈值（默认15） → 晋升老年代。

###### （2）老年代：对象存活时间长

- 存放长期存活的对象、大对象；
- 老年代满 → 触发**Major GC/Full GC（重量GC）**；
- 使用**标记-整理/标记-清除算法**；
- **Full GC 速度慢，会暂停业务线程（STW），应尽量避免**。
...
#### 五、关键概念：STW（Stop The World）

- **定义**：GC执行时，**暂停所有业务线程**，直到GC完成。
- **原因**：GC需要稳定的对象引用关系，不能一边清理一边创建对象。
- **优化方向**：现代GC（G1/ZGC）尽量缩短STW时间，保证系统响应速度。
...
JVM 提供不同收集器适配不同场景：
1. **Serial GC**：单线程，适合小型应用；
2. **Parallel GC**：多线程，JDK8 默认（吞吐量优先）；
3. **CMS GC**：低延迟，并发回收；
4. **G1 GC**：JDK9+ 默认，平衡吞吐量和延迟；
5. **ZGC/Shenandoah**：超低延迟，适合大内存、高响应场景。
...
1. 新对象 → Eden区；
2. Eden满 → Minor GC，存活对象进Survivor；
3. Survivor对象年龄达标 → 晋升老年代；
4. 老年代满 → Full GC，清理整个堆；
5. 全程自动执行，无需手动管理内存。
...
1. **核心目标**：自动回收无用内存，防止内存泄漏/溢出；
2. **判断垃圾**：可达性分析算法（以GC Roots为基准）；
3. **回收算法**：标记-清除、标记-复制、标记-整理；
4. **分代回收**：新生代用复制算法（Minor GC），老年代用整理算法（Full GC）；
5. **核心痛点**：STW会暂停业务，现代GC核心优化方向是缩短STW。
...
**内存泄漏**：程序已不再使用的对象，**本应被GC回收**，但因为**存在无效引用**，导致GC无法回收，对象一直占着堆内存；
随着时间推移，堆积越来越多，**内存占用持续飙升 → 频繁Full GC → OOM**。
...
1. **静态集合/静态变量**
   `static List/Map` 一直持有对象引用，永远不回收
2. **未关闭资源**
   IO流、数据库连接、Redis连接、Socket、线程池不关闭
3. **长生命周期持有短生命周期对象**
   全局集合缓存、全局Context持有局部对象
4. **匿名内部类/lambda**
   隐式持有外部类引用，导致外部类无法回收；如果匿名内部类 / Lambda 生命周期 **比外部类更长**，就会导致**外部类无法被 GC 回收**，造成**内存泄漏**。
5. **线程无限运行**
   非守护线程一直活着，持有业务对象
6. **ThreadLocal 使用不当**
   不调用`remove()`，线程复用导致对象一直残留
7. **缓存无淘汰策略**
   本地缓存Map、Guava缓存不设过期、不设最大容量
8. **循环引用**
   两个对象互相引用，旧版JVM引用计数无法回收（现在可达性分析可规避，但仍有隐患）

---

#### 三、标准排查流程（面试满分思路，按步骤说）

##### 步骤1：现象观察

- 服务器**内存占用持续走高，不下降**
- 频繁 **Full GC**，GC后内存释放很少
- 接口响应变慢、CPU飙升、最终 **OOM heap space**

##### 步骤2：获取堆快照

故障发生时导出堆dump：
1. `jmap -dump:format=b,file=heap.hprof 进程PID`
2. 或 JConsole、Arthas 在线dump

##### 步骤3：工具分析dump文件

常用工具：
- **MAT（Memory Analyzer）** 最常用
- **JProfiler、VisualVM**
- **Arthas** 在线实时排查，不用重启

##### 步骤4：定位泄漏点核心分析维度

1. **看大对象**：哪些对象实例数量极多、占用内存极大
2. **看对象引用链**：是谁一直引用着它，导致GC无法回收
3. **找可疑集合**：List/Map 里堆积大量无用对象
4. **分析线程**：线程是否卡死、常驻线程持有业务对象
5. **查看ThreadLocal**：是否残留大量对象未清理

##### 步骤5：代码定位与修复

根据引用链找到对应业务代码：
- 静态集合加**手动清理、过期淘汰**
- 资源使用后 **try-with-resources 自动关闭**
- ThreadLocal 用完必须 `remove()`
- 线程池合理设置核心线程、任务队列、拒绝策略
- 缓存加**过期时间、最大容量、LRU淘汰**
- 内部类避免长生命周期持有短生命周期对象

##### 步骤6：上线验证

压测 + 观察GC日志、内存曲线，确认内存平稳、无持续上涨。
...
1. **jps**：查看Java进程PID
2. **jstat -gc PID**：实时看GC次数、堆内存变化
3. **jmap**：查堆内存、导出dump
4. **jstack**：查线程死锁、线程阻塞
5. **Arthas**：`heapdump`、`dashboard`、`gc` 在线诊断
6. **MAT**：分析hprof，找泄漏根源、可疑大对象

---

#### 五、面试标准口述版（直接背）

先观察现象：**内存持续上涨、频繁Full GC、GC回收量小、最终OOM**；
先用`jps、jstat`确认GC异常，再用`jmap`导出堆快照，用MAT分析dump文件；
重点看**大对象、实例数量、对象引用链**，找出被无效引用无法回收的对象；
常见原因有**静态集合、资源未关闭、ThreadLocal未清理、线程常驻、缓存无淘汰、内部类引用**；
定位到代码后，通过**资源关闭、手动清集合、ThreadLocal remove、缓存加淘汰策略**修复，最后压测观察内存和GC曲线验证。
...
堆内存分：
**新生代 + 老年代**
- 新生代：Eden区 + Survivor0 + Survivor1（比例默认 8:1:1）
- 老年代：存放长期存活对象

调优核心：**合理划分新生代/老年代大小、选择垃圾收集器、控制GC频率、减少Full GC**
...
```
-Xmn    设置整个新生代大小
-XX:NewRatio=n   老年代:新生代 比例
-XX:SurvivorRatio=n  Eden:S0:S1 比例
```
- `-XX:NewRatio=2`  老年代:新生代 = 2:1
- `-XX:SurvivorRatio=8` Eden:S0:S1 = 8:1:1（默认）

##### 3. 元空间（JDK8+ 永久代取消，改用元空间）

```
-XX:MetaspaceSize=256m
-XX:MaxMetaspaceSize=512m
```
JDK8 无永久代，类元数据存在**本地内存**，不设上限容易耗尽本机内存，建议固定最大值。
...
```
- 栈越小：支持**线程数越多**
- 栈越大：递归、局部变量不易栈溢出 `StackOverflowError`

---

#### 三、垃圾收集器 常用参数

##### 1. Serial 串行收集器（单线程，客户端）

```
-XX:+UseSerialGC
```

##### 2. Parallel 并行收集器（JDK8 默认吞吐量优先）
...
##### 4. G1 收集器（JDK9+默认，大内存首选）

```
-XX:+UseG1GC
-XX:MaxGCPauseMillis=200  最大GC停顿时间
```
可预测停顿，适合**堆内存大、低延迟**服务
...
# 内存溢出自动导出dump
-XX:+HeapDumpOnOutOfMemoryError
-XX:HeapDumpPath=/usr/local/log/heap.hprof
```
作用：出问题看GC日志、OOM自动抓快照，方便排查泄漏。

---

#### 五、基础调优思路（面试直接说）

1. **固定堆大小**：-Xms = -Xmx，避免堆动态伸缩
2. **合理分配新生代**
   - 业务短命对象多 → 调大新生代
   - 长期对象多 → 适当减小新生代
3. **选择合适收集器**
   ...
   - 追求低延迟接口：CMS / G1
4. **限制元空间大小**，防止本地内存溢出
5. **配置GC日志 + OOM自动dump**，方便故障排查
6. 观察 **YGC频率、Full GC次数、GC停顿时间**
   目标：**少Full GC、低停顿、内存平稳**
...
-Xloggc:/log/gc.log
-XX:+HeapDumpOnOutOfMemoryError
-XX:HeapDumpPath=/log/heap.hprof
```
...
JVM基础调优主要先**固定堆初始和最大值相等**，避免动态扩容；
合理设置新生代、元空间、线程栈大小；根据业务选择Parallel/CMS/G1垃圾收集器，控制GC停顿；
开启GC日志和OOM自动dump，线上通过观察YGC、Full GC频率和停顿时间，调整内存比例与收集器，减少GC开销、避免OOM。

需要我给你整理 **JVM调优常见面试5问+标准答案** 吗？
