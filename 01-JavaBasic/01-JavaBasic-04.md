...
**Spring IOC 底层核心就是靠 Java 反射实现的**；
没有反射，就没有 Spring 的 IOC 容器、Bean 实例化、依赖注入。
...
1. 运行时**获取类信息**、构造器、方法、属性
2. 运行时**new 对象**（不用 new 关键字）
3. 运行时**给私有属性赋值**
4. 运行时**调用任意方法**

##### 2. Spring IOC 是什么

IOC：控制反转，把对象创建、依赖管理交给 Spring 容器，程序员不用自己 `new` 对象。

---

#### 三、Spring IOC 底层哪些地方用到了反射（重点面试点）

##### 1. Bean 实例化（通过反射创建对象）

Spring 解析配置类/XML，拿到 **Bean 的全类名**：
- 底层用 `Class.forName()` 获取 Class 对象
- 反射调用**无参构造器** `getDeclaredConstructor().newInstance()`
- **不通过 new，完全靠反射造 Bean 对象**

##### 2. 依赖注入 DI（给属性赋值）

`@Autowired` 自动注入底层：
1. 反射获取类的 **Field 成员变量**
2. 暴力放开权限 `setAccessible(true)`
3. 反射 `field.set(bean, 依赖对象)` 给属性赋值

##### 3. 初始化方法、销毁方法调用

`@PostConstruct`、`init-method`、`destroy-method`：
- 反射拿到目标 Method
- `method.invoke(bean)` 运行时调用

##### 4. AOP 动态代理底层

JDK 动态代理、Cglib 代理，**全部依赖反射**做方法拦截、增强调用。

##### 5. 配置绑定、注解解析

Spring 扫描 `@Component`、`@Service` 等注解：
- 反射读取类、方法、属性上的注解信息
- 判断是否要交给 IOC 管理
...
1. Spring 启动扫描包，读取所有带注解的类；
2. 通过**反射 Class 对象**解析类结构、注解、构造器、属性；
3. 利用**反射构造器**创建 Bean 实例，存入 IOC 容器单例池；
4. 再通过**反射 Field** 给 Bean 的属性做依赖注入；
5. 最后反射调用初始化方法，完成整个 IOC 容器初始化。

---

#### 五、IOC 为什么必须用反射？

1. 程序**编译期不知道要创建哪个类**，类名是配置/注解动态读取的，只能运行时靠反射；
2. 不能硬编码 `new UserService()`，做不到通用容器；
3. 需要**绕过权限**给私有属性注入值，只有反射能做到；
4. 需要统一批量创建对象、统一管理生命周期，反射是唯一底层支撑。
...
- 缓存反射的 Class、Method、Field 对象，不重复解析
- 大量使用 **Bean 后置处理器、元数据缓存**
- 高版本 Spring 优化反射调用性能
...
Spring IOC 底层**基于 Java 反射机制实现**。
Spring 启动时通过反射获取类的字节码对象，解析注解和配置；通过反射构造器实例化 Bean，放入 IOC 容器；再通过反射给成员属性做依赖注入，同时利用反射调用初始化和销毁方法。
简单说：**反射是底层技术，IOC 是反射的经典落地应用**。

---

要不要我给你画一张**Spring IOC 底层反射执行流程图**，一看就懂？

### 反射调用方法

---

#### 一、反射调用方法 **完整标准 6 步**（含结果转换）

1. 获取目标类 **Class** 对象
2. 通过反射**创建类实例**
3. 获取目标 **Method** 对象（指定方法名+参数类型）
4. 私有方法则开启 `setAccessible(true)`
5. **method.invoke(实例, 入参)** 执行方法，返回 `Object`
6. **把 Object 结果强制转为真实业务类型** 使用
...
public class ReflectTest {
    public static void main(String[] args) throws Exception {
        ...
        // 3. 获取有参方法
        Method createMethod = clazz.getDeclaredMethod("createOrder", String.class);

        // 4. 执行方法，返回Object
        Object resultObj = createMethod.invoke(serviceObj, "ORD20260518");

        // 5. 【关键缺失的一步】强制类型转换
        String orderResult = (String) resultObj;
        System.out.println(orderResult);


        // 再演示返回Integer
        Method countMethod = clazz.getDeclaredMethod("getOrderCount");
        Object countObj = countMethod.invoke(serviceObj);
...
1. **invoke 方法统一返回 Object 类型**，因为反射不知道方法实际返回什么；
2. 必须**手动向下强转**为真实返回类型（String、Integer、自定义实体类等）；
3. 如果不转换，只能拿到 Object，无法调用业务属性和方法；
4. 若方法是 **void**，invoke 返回 `null`，不需要转换。
...
反射调用完整步骤：
1. 先获取目标类的 Class 字节码对象；
2. 通过反射构造器创建类的实例对象；
3. 根据方法名和参数类型获取 Method 方法对象；
4. 若是私有方法，设置 setAccessible(true) 放开权限；
5. 调用 invoke 传入实例和入参执行方法，返回 Object；
6. **将返回的 Object 强制转换成方法实际返回类型**，完成业务逻辑使用。

---

需要我给你写一个**通用反射工具类**，自动反射调用、自动泛型转结果，不用每次手动强转吗？
...
1. **BigInteger**
   处理**超大整数**，超出 `long` 范围（`long` 最大：`9223372036854775807`）。
2. **BigDecimal**
   处理**高精度小数**，解决 `double/float` 浮点丢失精度问题，**金融、金额、计算必用**。
...
// 快速创建
BigDecimal d3 = BigDecimal.valueOf(0.2);
```
...
// 除法必须指定 保留位数+舍入模式，否则报错
BigDecimal div = a.divide(b, 2, RoundingMode.HALF_UP);
```

##### 3. 重点：除法必写舍入模式

不写会抛出：`ArithmeticException: Non-terminating decimal expansion`
常用舍入模式：
- `RoundingMode.HALF_UP`：**四舍五入**（最常用，金额）
...
```
- `equals` 会比较**小数位数**，`new BigDecimal("1.0").equals(new BigDecimal("1")) → false`
...
BigDecimal res = num.setScale(2, RoundingMode.HALF_UP);
// 结果：1.23
```
...
String strVal = bigDecimal.toString();
```
...
1. **不可变类**
   每次加减乘除都会**生成新对象**，原对象不变。
2. 性能比基础类型慢，适合**金额、超大数**，普通计算不用。
3. BigDecimal 解决浮点精度丢失，是金融项目强制规范。
...
1. **BigInteger**：超大整数，替代 long。
2. **BigDecimal**：高精度小数，金额专用，解决 double 精度丢失。
3. 构造优先用**字符串**，禁止 double 直接构造。
4. 除法必须指定**保留位数+舍入模式**。
5. 比较用 `compareTo`，不要用 `equals`、`==`。
6. 都是不可变类，运算结果需接收返回值。

需要我给你写一段 **0.1+0.2 用BigDecimal正确计算** 的完整测试代码吗？

#### BigInteger & BigDecimal 底层原理

核心共性：**不可变、数组存数据、十进制精准运算、无二进制浮点误差**
...
1. **不可变类**
   所有运算 `add/subtract/multiply` 不会修改原对象，**每次返回全新对象**。
   原因：内部数值私有+无set方法，线程安全、适合缓存。

2. **不使用二进制存储**
   int/long/double 都是**二进制存储**，导致小数精度丢失；
   BigInteger/BigDecimal 基于**十进制数组**存储，天然精准。

---

##### 二、BigInteger 底层原理（超大整数）

###### 1. 核心存储结构

```java
// 源码核心属性
final int[] mag;   // 存储大数字【绝对值】，按高位→低位分段存储
final int signum;  // 符号位：1正数  -1负数  0=数字0
```
- 超大数字拆分成**int数组**分段保存，突破 long 最大值限制；
- 不再用基本类型存数值，理论无限大。
...
基本类型有固定bit长度；
BigInteger 靠 **动态int数组** 扩容存储，无上限。

---

##### 三、BigDecimal 底层原理（高精度小数）

###### 1. 核心存储结构

```java
// 源码核心属性
private final BigInteger intVal; // 完整无缩放整数
private final int scale;         // 小数位数（缩放值）
private final int precision;     // 总有效位数
```
###### 存储逻辑（关键）

把**小数 → 纯整数**存储，彻底避开二进制浮点：
例：`3.14`
- intVal = 314
- scale = 2
真实值计算公式：
\[
- 数值 = $$\frac{intVal}{10^{scale}}$$
\]

###### 2. 精度丢失根源对比

- double：二进制科学计数法存储 → 0.1 无法精准表示
- BigDecimal：十进制整数+缩放 → 0.1 就是 1 / 10，**完全精准**

###### 3. 运算底层逻辑

1. 加减法：统一对齐 scale 小数位，转成大整数运算，再还原缩放
2. 乘法：整数相乘，scale 相加
3. 除法：**无限循环小数必须指定舍入模式**，否则直接抛异常

---

##### 四、高频面试点

###### 1. 为什么 BigDecimal 不用 equals 比较？

`equals` 同时比较 **数值 + scale**
- new BigDecimal("1.0")  scale=1
- new BigDecimal("1")    scale=0
👉 equals = false，但数值相等
✅ 正确：`compareTo()` 只比数值

###### 2. 为什么除法必须加舍入模式？

二进制/十进制无限小数（1÷3），无法用有限数组存储，
不指定舍入策略 → 直接抛出 `ArithmeticException`

###### 3. 为什么不推荐 new BigDecimal(double)？

double 本身已经二进制失真，传入时误差已经存在，无法挽回；
✅ 推荐：`new BigDecimal("0.1")` / `BigDecimal.valueOf(0.1)`

---

##### 五、底层性能缺点

1. 基于数组+对象运算，**远慢于 int/long/double**
2. 每次运算新建对象，内存开销大
3. 只适合：金额、高精度计算、超大数；普通业务禁用

---

##### 六、一句话速记

1. **BigInteger**：int数组存大数+符号位，模拟手工数学运算，解决超大整数。
2. **BigDecimal**：大整数+小数位数scale，小数转整数存储，根治浮点精度丢失。
3. 两者都是**不可变**，运算产生新对象，适合金融、高精度场景。

#### BigDecimal/BigInteger 应用场景



##### 一、一句话区分

- **BigDecimal**：**金融小数**（金额、利率、汇率、手续费）
- **BigInteger**：**超大整数**（无小数、远超 long 范围）
...
```
一旦数字**超过这个长度**，普通整型全部存不下，只能用 `BigInteger`。

---

##### 三、BigInteger 真实业务用途（面试+开发）

###### 1. 超大序列号/流水号/唯一ID

- 订单号、设备唯一编码、雪花算法极端扩容场景
- 物联网设备ID、硬件序列号，数字超长
- 银行/政务超大流水编号，纯整数、无小数
...
- 平台积分、用户累计签到次数、超大点赞量
- 库存、票数、投票统计，数值不断累加怕溢出
- 电商秒杀超大并发计数

###### 5. 数据库超大整型字段映射

- 数据库 `NUMERIC / DECIMAL` 存超大整数
- 后端不用 Long，用 BigInteger 接收，防止溢出
...
数字爆炸式增长，**long 直接溢出**，只能 BigInteger
```java
...
| 类型       | 适用             | 核心场景                               |
| ---------- | ---------------- | -------------------------------------- |
| BigDecimal | 带小数、高精度   | 支付、账单、利率、财务、金融计算       |
| BigInteger | 超大整数、无小数 | 加密、大数ID、阶乘、科学计算、超大计数 |

---

##### 六、面试标准答案

1. `BigDecimal` 解决 **double 浮点精度丢失**，专门用于金融金额、小数运算。
2. `BigInteger` 是**任意长度大整数**，解决 long 溢出问题，
多用于**加密算法、超大序号、大数数学运算、海量计数**等纯整数场景。

需要我给你写一段 **RSA 大数取模 + BigInteger 实操代码** 吗？一看就懂。
...
面试官您好！这几个概念是 Java 8 引入的核心特性，核心目标是**简化代码、提升开发效率，实现函数式编程风格**，其中

**函数式编程是思想，函数式接口是基础，Lambda 是语法糖，Stream 是函数式编程在集合处理上的落地实现**，四者层层递进、紧密关联。下面我分别解释：
...
- 普通流：**串行流**，单线程逐个执行
- 并行流：**多线程并行执行**，底层默认使用 **ForkJoinPool 线程池**
- 开启方式：
  1. 集合直接调用 `.parallelStream()`
  2. 普通流调用 `.parallel()` 转为并行流
...
##### 坑1：操作**有状态/共享变量** → 线程安全问题

并行流多线程同时操作同一个集合/变量，会并发异常
```java
// ❌ 错误：ArrayList 线程不安全
List<Integer> resList = new ArrayList<>();
list.parallelStream().forEach(resList::add);

// ✅ 正确：用线程安全集合 / collect 收集
List<Integer> res = list.parallelStream()
        .collect(Collectors.toList());
```

##### 坑2：包含 IO、数据库、Redis 操作 → 不建议

并行流**默认共用全局 ForkJoinPool**，IO 阻塞会耗尽公共线程，影响全局所有并行任务。

##### 坑3：顺序依赖业务不能用

例如：需要按顺序累加、排序强依赖顺序，并行会错乱。

##### 坑4：简单任务并行反而更慢

线程创建、任务拆分有开销，**小数据量、简单计算**，串行更快。

---

#### 七、底层原理（面试）

1. 并行流底层：`ForkJoinPool` + `ForkJoinTask`
2. 任务拆分：大数据集合拆分成多个小任务
3. 多线程并行执行
4. 最后合并结果
5. 默认线程数：`Runtime.getRuntime().availableProcessors()` CPU核心数
...
pool.submit(() -> {
    list.parallelStream().forEach(xxx);
}).join();
```

需要我给你写一段**并行流线程安全报错+修复**的完整测试代码吗？



### ForkJoin池

**默认不用自定义 ForkJoinPool**，`parallelStream` 底层**自带公共ForkJoin池**，开箱即用。
...
list.parallelStream()
```
- 底层默认使用：**`ForkJoinPool.commonPool()`**
- 线程数默认：**CPU核心数 - 1**
- 全局共享一个公共池，**所有并行流共用**
- 无需自己创建、初始化、管理线程池
...
#### 2. 什么时候才需要**自定义 ForkJoinPool**

3种必自定义场景：
1. **不想共用全局公共池**，怕被其他并行流挤占线程
2. 需要**修改并行度（线程数）**
3. 并行流里有**阻塞IO、sleep、慢查询**，防止拖垮全局公共池

---

#### 3. 如何自定义 ForkJoinPool 执行 parallelStream

##### 原理

并行流会**继承当前线程的ForkJoin池**，所以把流放进自定义池里执行即可。

```java
// 1. 自定义池，指定并行度（线程数）
ForkJoinPool customPool = new ForkJoinPool(10); // 10个线程
...
| 方式               | 线程池         | 并行度   | 优缺点                         |
| ------------------ | -------------- | -------- | ------------------------------ |
| 直接parallelStream | 公共commonPool | CPU核-1  | 最简单，全局共享，易被阻塞拖慢 |
| 自定义ForkJoinPool | 自己新建池     | 自己指定 | 隔离任务、可控并发、不影响全局 |

---

#### 5. 开发规范

- **纯CPU密集、无阻塞**：直接用默认 `parallelStream`，**不用自定义池**
- **有IO阻塞、接口调用、sleep**：**必须自定义ForkJoinPool**，避免占满公共池
- 业务核心并行计算：建议自定义池隔离，互不干扰

---

#### 6. 面试高频答案

1. `parallelStream` 默认使用 **ForkJoinPool.commonPool**，**不需要手动定义**。
2. 默认并行度由JVM参数 `common.parallelism` 决定，默认CPU核心数减一。
3. 存在阻塞任务、需要调并发数、任务隔离时，**手动创建ForkJoinPool包裹并行流执行**。
4. 不建议在并行流里写阻塞代码，会耗尽公共池线程。

需要我给你写一段**默认池 vs 自定义池**对比测试代码吗？

### 普通操作、Stream、parallelStream() 全面对比

我用**最通俗、最实用**的方式给你讲清楚三者的区别、性能、使用场景，直接覆盖开发 99% 的场景。

#### 一、先一句话总结

1. **普通操作（for/foreach）**：基础循环，单线程，简单直接，适合小数据量
2. **Stream**：流式操作，单线程，代码优雅，支持链式过滤/映射/聚合
3. **parallelStream()**：并行流，**多线程**，利用多核CPU，大数据量下速度最快

---

#### 二、核心区别对比表

| 特性               | 普通for/foreach  | Stream()               | parallelStream()     |
| ------------------ | ---------------- | ---------------------- | -------------------- |
| **线程模型**       | 单线程           | 单线程                 | **多线程（自动）**   |
| **代码简洁度**     | 繁琐             | 优雅、链式             | 同Stream             |
| **性能（小数据）** | 最快             | 稍慢                   | 最慢（线程开销大）   |
| **性能（大数据）** | 慢               | 中等                   | **最快**             |
| **线程安全**       | 安全             | 安全                   | **不安全（需注意）** |
| **顺序**           | 严格有序         | 严格有序               | **无序**             |
| **适用场景**       | 简单逻辑、小数据 | 优雅处理集合、过滤排序 | 大数据量、无状态计算 |

---

#### 三、代码示例对比（同一个需求：筛选+计算）

需求：从 1~10000 的数字中，找出偶数，然后求和。
...
```
✅ 优点：**一行链式搞定，可读性极强**  
✅ 支持：filter、map、sorted、collect、reduce 等
...
int sum = list.parallelStream()
              ...
              .sum();
```
✅ 优点：**自动多线程，大数据量速度碾压前两者**  
⚠️ 底层自动使用 ForkJoinPool 线程池
...
**普通 for > Stream > parallelStream**  
原因：并行流创建线程、切换线程的开销，比计算本身还大。

##### 2. **数据量大（>1万 ~ 100万+）**

**parallelStream >>> Stream > 普通 for**  
多核CPU下，并行流能把速度提升 **N 倍**（N≈CPU核心数）。

---

#### 五、最关键的坑：parallelStream 必须注意

##### 1. 线程不安全

不要在并行流里修改**外部变量**（如集合、计数器），会出现数据错乱。

❌ 错误写法：
```java
List<Integer> res = new ArrayList<>();
list.parallelStream().forEach(num -> res.add(num)); // 线程不安全！
```

✅ 正确写法：
```java
List<Integer> res = list.parallelStream().collect(Collectors.toList());
```

##### 2. 不保证顺序

parallelStream 是**无序**的，依赖顺序的业务不要用。

##### 3. 不要用在阻塞/IO操作

并行流默认使用公共线程池，IO 阻塞会拖慢整个应用。
...
1. **普通循环**：简单、单线程、小数据最快
2. **Stream**：优雅、单线程、适合集合链式处理
3. **parallelStream**：多线程、大数据性能王者，但要注意线程安全与无序
4. 选择核心：**看数据量 + 看是否需要并行 + 看是否要求有序**

需要我给你做一个**100万数据的真实性能测试代码**，你直接运行就能看到三者速度差距吗？