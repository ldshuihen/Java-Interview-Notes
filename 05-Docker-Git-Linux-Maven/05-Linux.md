...
实时监控**整机CPU、内存、负载、所有进程**运行状态，Linux最常用性能排查命令。
#### 2. 执行
...
```
top - 15:30:00 up 20 days, 2 users,  load average: 0.20, 0.30, 0.25
Tasks: 120 total, 1 running, 119 sleeping
%Cpu(s):  1.0 us, 0.5 sy, 0.0 ni, 98.0 id, 0.3 wa
MiB Mem : 15800 total, 8000 free, 4000 used, 3800 buff/cache
MiB Swap: 8192 total, 8192 free, 0 used.
```
- **load average: 1分钟、5分钟、15分钟平均负载**
  规则：**数值 ≤ CPU核心数 负载正常；远超核心数 = 系统拥堵**
- **%Cpu(s)**
  - `us` 用户态CPU（业务代码占用）
  - `sy` 内核态CPU
  - `id` 空闲CPU（越低越忙）
...
#### 二、字段含义（新版 Linux 统一标准）

```
              total        used        free      shared  buff/cache   available
内存总大小     总内存      已使用内存   空闲内存    共享内存    缓冲缓存    可用内存
```
...
2. **used**
**已经被程序实实在在占用的内存**（进程运行占用）

3. **free**
**完全没被使用的裸空闲内存**
> 现在 Linux 系统 free 很小是正常现象
...
1. **Linux 设计思想**
内存宁可闲置也不用，**尽量全利用做缓存**，所以 free 很小很正常。

2. **缓存会自动释放**
当应用需要内存时，系统会**立刻回收 buff/cache** 分给程序使用。

3. **判断内存不足标准**
- 看 `available` 持续走低
- `si so` 频繁变化（vmstat 看到）
- 大量使用 Swap 交换分区
...
- available 充足就没事
- Swap 一涨立刻查内存泄漏
- 程序卡顿优先看 available + Swap

#### 八、面试简答

问：free 命令中 free 很小是不是内存满了？
答：**不是**，Linux 会把空闲内存用作文件缓存，真正看剩余内存看 `available`，缓存内存可随时释放给进程使用。

需要我给你整理**Linux 内存排查完整流程**吗？
...
查看**CPU、内存、磁盘交换、运行队列**整体系统压力，轻量高效。
#### 2. 常用命令
...
- `r`：**运行队列进程数**（大于CPU核数代表CPU压力大）
- `b`：阻塞等待IO的进程数
- `free`：空闲内存
- `si`：**swap读**（磁盘内存写入内存）
- `so`：**swap写**（内存写入虚拟磁盘）
> **si/so 长期不为0 = 内存不足，频繁使用交换分区，系统卡顿**
- `us/sy/id/wa`：同top CPU状态
...
专门排查**磁盘读写瓶颈**，服务器卡顿80%都是IO问题。
#### 2. 常用命令

```bash
iostat -x 1   # 每秒刷新，展示详细IO统计
```
#### 3. 核心关键指标

- `%util`：**磁盘设备繁忙率**
  - 接近100%：磁盘IO打满，严重瓶颈
- `await`：IO请求平均等待耗时（单位ms）
  - 大于10ms：IO延迟高，业务变慢
- `rMB/s`：每秒读取速度
- `wMB/s`：每秒写入速度
- `rareq-sz`/`wareq-sz`：平均读写请求大小

#### 4. 排查场景

接口超时、数据库慢、日志狂打、文件批量读写 → 优先看iostat
...
定位日志进程、MySQL、ES、备份脚本、文件同步脚本占用磁盘IO

---

### 六、sar 系统历史性能日志（最强统计工具）

#### 1. 作用

**既能实时监控，又能查历史性能数据**，复盘凌晨卡顿、高峰期压力必备。
#### 2. 安装
...
`当前时间 + 开机时长 + 在线用户 + 1/5/15分钟负载`
**运维快速判断服务器压力首选命令**

---

### 八、Java 服务 高CPU/高内存 精准排查（面试重点）
...
- CPU高：死循环、频繁GC、正则卡死、大量循环计算
- 内存高：内存泄漏、堆参数过小、大对象常驻
...
find / -size +100M
```
常见问题：日志爆满、core dump文件、缓存文件占满磁盘
...
- 连接数过多：端口耗尽、TIME_WAIT堆积
- 带宽打满：文件传输、视频、日志推送
- 丢包延迟：机房网络、跨区域调用慢

---

### 十一、线上性能排查标准流程（工作实战流程）

1. 先用 `uptime` 看整体负载
2. `top` 确认是CPU高、内存高还是IO高
3. CPU高：jstack 排查Java线程
4. 内存高：jmap 分析堆内存
5. 卡顿无资源占用：`iostat` 查磁盘IO
6. 网络慢：`iftop` + `sar -n DEV` 查流量
7. 历史问题复盘：使用 `sar` 查过往性能

需要我把这份内容整理成**面试背诵精简版**，或者做成**一页纸速查命令表**吗？
