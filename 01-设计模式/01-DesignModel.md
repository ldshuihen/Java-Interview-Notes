...
**单例模式属于创建型模式，保证一个类在整个项目中**仅有一个实例**，并提供一个全局访问点。**
...
2. 连接池、线程池、缓存工具
3. 日志工具、唯一序列号生成器
4. 项目中**统一工具类、全局常量管理**都常用

##### 三、六大实现方式（面试重点）

###### 1. 饿汉式（常用、线程安全）

类加载直接创建实例，**天生线程安全**，缺点类初始化就占用内存
```java
...
Spring Bean 默认就是**单例**，容器统一管理实例

##### 四、破坏单例的三种方式 & 解决

1. **反射** → 枚举可杜绝
2. **序列化反序列化** → 重写readResolve / 用枚举
3. **克隆** → 重写clone禁止克隆

##### 五、项目实战（剧院票务系统原话）

我在项目里多处用到单例：
1. **全局配置读取工具类**使用静态内部类单例
2. **Redis连接工具、Minio文件上传工具**做成单例，避免频繁创建销毁连接
3. 项目中**常量管理类、雪花算法ID生成器**也采用单例
4. Spring容器中所有业务Service默认也是**容器单例**，节省资源提升效率
...
1. 职责过重，违背单一职责
2. 无接口，扩展困难
3. 并发场景使用不当容易出现线程安全问题

##### 七、面试极简30秒背诵

单例模式保证类全局只有一个实例，提供统一访问入口。
常用实现有饿汉、懒汉、DCL双重校验锁、静态内部类、枚举。
其中**静态内部类性能最优**，**枚举最安全**。
我项目中将工具类、连接工具、ID生成器设计为单例，减少对象创建开销，统一管理全局资源。

##### 高频问答

1. **为什么DCL要加volatile？**
禁止JVM指令重排序，防止多线程拿到半初始化对象。

2. **静态内部类为什么线程安全？**
静态内部类只会在调用时加载，类加载过程JVM保证线程安全。

3. **Spring Bean是单例还是多例？**
默认**单例**，request、session作用域为多例。
...
| 关键字                         | 原因说明                                                     |
| ------------------------------ | ------------------------------------------------------------ |
| **`private`**                  | **防止外部直接访问**这个实例。单例模式要求只能通过 `getInstance()` 方法获取实例，不能让外面直接 `HungrySingleton.INSTANCE` 来拿到对象。 |
| **`static`**                   | **最关键**。因为这个变量必须在**类加载时**就被创建（饿汉式的核心思想）。只有 `static` 成员变量才会在类加载阶段初始化。 |
| **`final`**                    | 1. 表示这个引用一旦被初始化，就**不允许再被修改**。 2. 保证单例的唯一性，防止后续代码意外重新赋值。 3. 给 JVM 一些优化空间。 |
| **`INSTANCE`**                 | 大写命名规范，静态常量通常使用**全大写** + 下划线命名。      |
| **`= new HungrySingleton();`** | **类加载时立即执行**，在 `HungrySingleton` 这个类被 JVM 第一次加载时就创建实例，属于“饿汉式”初始化。 |
...
- **final** 的含义是：**变量一旦被赋值，就不允许再修改**。
- **懒加载（Lazy Loading）** 的核心是：**第一次调用 getInstance() 时才创建实例**，在此之前 INSTANCE 是 null。

如果加上 final，你就必须在**声明变量时就初始化**（即 = new XXX()），这样就变成了**饿汉式**，而不是懒加载了。
...
**工厂模式就是把对象的创建逻辑统一抽离到工厂类中，业务代码只负责使用对象，不负责new对象，解耦创建与使用。**

##### 2. 核心作用

1. **降低耦合**：业务层不用关心对象怎么创建
2. **统一管理**：对象创建逻辑集中维护
3. **符合开闭原则**：新增产品不用改原有业务代码
4. **便于扩展**：批量生成同类对象
...
- 一个工厂类，根据**类型参数**创建不同对象
- 优点：简单好用
- 缺点：新增产品要改工厂代码，违反开闭

###### 2）工厂方法模式

- **一个产品对应一个工厂**，每个工厂只造一种产品
- 遵循开闭原则，新增产品直接新增对应工厂
- 缺点：类数量爆炸，类太多
...
- 生产**一整套相关产品族**，不止单个对象
- 适合成套产品场景，比如手机+耳机整套配件

##### 4. 你项目实战用法（必说，加分）

我在**剧院票务系统支付模块**用到**简单工厂+策略模式结合**
微信支付、支付宝支付、余额支付属于不同支付产品
通过**支付工厂**根据支付类型，统一创建对应的支付实现类
业务层直接调用统一支付接口，不用手动new不同支付类
后续新增云闪付等支付方式，扩展非常方便。

##### 5. 面试官高频简答

**问：工厂模式和 new 区别？**
答：直接new耦合太高，创建逻辑散落在业务里不好维护；工厂统一创建，集中管控，后期扩展、修改更方便。
...
工厂模式是创建型设计模式，核心是**将对象创建与业务使用分离**，由专门工厂类统一实例化对象。
分为简单工厂、工厂方法、抽象工厂三种。
我项目支付模块使用简单工厂，根据支付类型生成对应支付实现类，简化代码、方便扩展，降低业务层耦合度。
...
**观察者模式属于行为型模式**，定义一对多依赖关系：**一个主题（被观察者）状态发生变化，所有依赖它的观察者都会自动收到通知并触发更新**。

##### 二、核心角色

1. **Subject 被观察者/主题**：维护观察者列表，提供新增、移除、通知观察者方法
2. **Observer 观察者**：统一更新接口，接收通知后执行自身业务
3. **具体被观察者**：状态变更，主动推送消息
4. **具体观察者**：实现更新逻辑，响应事件
...
我在项目**订单模块**用到观察者模式：
订单支付成功（被观察者状态改变），自动触发多个观察者业务：
1. 短信通知观察者：发送购票成功短信
2. 站内信观察者：推送平台消息
3. 积分观察者：自动发放观影积分
4. 库存观察者：更新座位最终售卖状态

不用在支付成功后硬编码一堆业务代码，新增通知业务只新加观察者即可，代码清爽易扩展。

##### 六、Java 原生实现

JDK 自带：`java.util.Observable`（被观察者） + `Observer`（观察者）
现在开发更多自己手写接口实现，更灵活。
...
- **观察者**：主题和观察者**直接通信**，强关联，在同一进程内多用
- **发布订阅**：中间多一层**消息队列/调度中心**，双方完全解耦，跨服务、分布式常用（如MQ）
...
观察者模式是行为型设计模式，建立一对多监听关系。被观察者状态改变，自动通知所有观察者执行更新操作。
核心解耦事件发布与事件处理，适合订单状态变更、消息推送等场景。我项目中用它解耦订单支付成功后的短信、积分、消息推送等联动业务，扩展性很强。
